...
async function handleSignup(btn) {
    ...
    try {
        ...
        const { data: student } = await supabase
            .from('students')
            .select('id, cin, email, full_name, grade, td_group')
            .eq('cin', currentSignupCIN)
            .single();

        localStorage.setItem('attendly_user', JSON.stringify({
            id: student.id, cin: student.cin, email: student.email,
            full_name: student.full_name, grade: student.grade,
            td_group: student.td_group, role: 'student'
        }));
        openCurtainsAndRedirect('student/index.html');
    } catch (err) {
...
async function handleLogin(btn) {
    ...
    try {
        const { data, error } = await supabase
            .from('students')
            .select('id, cin, email, full_name, password, grade, td_group, is_registered')
            .eq('email', email)
            .maybeSingle();
        ...
        localStorage.setItem('attendly_user', JSON.stringify({
            id: data.id, cin: data.cin, email: data.email,
            full_name: data.full_name, grade: data.grade,
            td_group: data.td_group, role: 'student'
        }));
        openCurtainsAndRedirect('student/index.html');
    } catch (err) {
